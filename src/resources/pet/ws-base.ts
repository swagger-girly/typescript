...
import { PetEmitter, PetStreamMessage, WebSocketError, buildURL } from './internal-base';
...
export interface PetWSParameters extends Record<string, unknown> {
  petId: string;
}

export interface PetWSReconnectOptions {
  /**
   ...
   */
  onReconnecting(event: ReconnectingEvent<PetWSParameters>): ReconnectingOverrides<PetWSParameters> | void;
...
export interface PetWSBaseOptions {
  /**
   ...
   */
  reconnect?: PetWSReconnectOptions | null | undefined;
...
export abstract class PetWSBase<TSocket extends WebSocketLike> extends PetEmitter {
  url!: URL;
  socket!: TSocket;

  protected _client: HelloWorldTestingggg;
  protected _parameters: PetWSParameters | null | undefined;
  private _reconnectOptions: PetWSReconnectOptions | null;
  private _sendQueue: SendQueue<PetAPI.ConnectClientEvent>;
  ...
  constructor(
    ...
    options?: PetWSBaseOptions | undefined,
  ) {
    super();
    this._client = client;
    this._parameters = parameters ?? undefined;
    this._reconnectOptions = options?.reconnect ?? null;
    this._sendQueue = new SendQueue<PetAPI.ConnectClientEvent>(options?.maxQueueSize);
  }
  ...
  send(event: PetAPI.ConnectClientEvent) {
    if (!this.socket) {
      throw new HelloWorldTestinggggError(
        'Internal error: failed to initialize socket. Please report this issue.',
      );
    }
   ...
   * ```
   */
  stream(): AsyncIterableIterator<PetStreamMessage> {
    return this[Symbol.asyncIterator]();
  }

  [Symbol.asyncIterator](): AsyncIterableIterator<PetStreamMessage> {
    if (!this.socket) {
      throw new HelloWorldTestinggggError(
        'Internal error: failed to initialize socket. Please report this issue.',
      );
    ...
    const queue: PetStreamMessage[] = [];
    const resolvers: (() => void)[] = [];
    let done = false;
    let currentSocket = this.socket;

    const push = (msg: PetStreamMessage) => {
      queue.push(msg);
      resolvers.shift()?.();
    };

    const onEvent = (event: PetAPI.ConnectServerEvent) => {
    ...
    const onReconnecting = (evt: ReconnectingEvent<PetWSParameters>) => {
    ...
    const onClose = (code: number, reason: string, unsent: UnsentMessage<PetAPI.ConnectClientEvent>[]) => {
    ...
    const resolve = (res: (value: IteratorResult<PetStreamMessage>) => void) => {
      if (queue.length > 0) {
    ...
    const next = (): Promise<IteratorResult<PetStreamMessage>> =>
      new Promise((res) => {
  ...
  private _connect(): TSocket {
    ...
    socket.on('message', (data: string | ArrayBuffer | ArrayBufferView, isBinary: boolean) => {
      ...
      let event: PetAPI.ConnectServerEvent;
      try {
        event = JSON.parse(text) as PetAPI.ConnectServerEvent;
      } catch {
        this._emit('raw', data);
        return;
      }
  ...
  private async _reconnect(closeCode: number): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      let reconnectingEvent: ReconnectingEvent<PetWSParameters> = {
        attempt,
      ...
      let overrides: ReconnectingOverrides<PetWSParameters> | void;
      try {
